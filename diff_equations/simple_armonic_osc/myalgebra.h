...
template <typename T> std::vector<T> operator+(const std::vector<T> &a, const std::vector<T> &b) {
    ...
    assert(a.size() == b.size()); // restituisce un messaggio di errore
    std::vector<T> result(a.size()); //sto usando un costruttore di
                                   //vector che accetta in input un unsigned int
